...
os.path.exists('../resources/data/series.csv')

# Importing data

series = pd.read_csv('resources/data/series.csv', sep = ',',delimiter=',') # Remove this to be modular
#ratings = pd.read_csv('resources/data/ratings.csv') # Remove this to be modular
series.fillna('')

# Required columns - Title and movie plot
content = series[["title", "description"]]           
...
#def recommendations(title, cosine_sim = cos_sim): 
    ...
    top_10_shows = list(similarity_scores.iloc[1:11].index)
    # Removing chosen movies
    top_indexes = np.setdiff1d(top_10_shows,[idx_1,idx_2,idx_3]) 
    for i in top_shows: 
        recommended_shows.append(list(content.index)[i]) 
